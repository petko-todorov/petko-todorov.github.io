...
const TargetCursor = ({
    ...
    useEffect(() => {
        ...
        const moveHandler = (e) => {
            ...
            const targetElement = elementUnderMouse?.closest(targetSelector);

            if (targetElement && !isOverTarget) {
                // Mouse entered a target
                isOverTarget = true;
                document.body.style.cursor = 'none';
                gsap.to(cursor, { opacity: 1, duration: 0.2 });
            } else if (!targetElement && isOverTarget) {
                // Mouse left all targets
                isOverTarget = false;
        ...
        // Click animation - only when over target
        const mouseDownHandler = (e) => {
            if (!dotRef.current || !isOverTarget) return;
            gsap.to(dotRef.current, { scale: 0.7, duration: 0.3 });
            gsap.to(cursorRef.current, { scale: 0.9, duration: 0.2 });
        };
